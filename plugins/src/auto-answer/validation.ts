import { getQuestions, getUnansweredQuestionsBySource } from './storage';
...
/**
 * Check if source ID has any questions in database
 */
export function validateSourceIdExists(sourceId: string): boolean {
  const questions = getUnansweredQuestionsBySource(sourceId);
