    ...
    return http.createServer(async (req, res) => {
        ...
        try {
            ...
            if (method === "POST" && url === "/api/answers") {
                ...
                const allQuestionsInGroup = db
                    .prepare("SELECT id FROM questions WHERE group_id = ? AND source_id = ?")
                ...
                for (const q of allQuestionsInGroup) {
                    if (!answeredQuestionIds.has(q.id)) {
                        ...
                        return sendJson(res, 400, {
                            error: `Must answer all questions in the group. Missing question: ${q.id}`,
                        });
                    }
                }
